...
Reference outputs in `expected-exiftool/` are pre-generated by `generate-exiftool-expected.sh` and committed, so tests don't require exiftool at runtime.

Test metadata is documented in `metadata-ground-truth.csv`, with values sourced from exiftool.
